...
%
%   DR 10/2019 & VB 5/2020

% parameters
p.ddir = '/mnt/sdb1/CCDT/eeg/'; % data directory
p.subj = []; %'HUP191'; % subject (leave empty to batch process all subjects in database)
...
sI = 1; % use all session times (1) or just first one (0)?
wavl = 'morl'; % wavelet
...
svnm = 'powRT_040425_CAR_hg70110'; %'powRT_sIall_500mspreCUE'
debug = 0; % debug plots? (0 or 1)
odir = '/mnt/sdb1/CCDT/procData072023/'; %'/Volumes/CCDT/procDataFiles/newProcData/'; % output directory
...
%             P = zscore(P,[],'all'); % normalize to account for different electrode impedances
        else
            P = cwt(dat(:,ii),scales,wavl);
            P = abs(P); %  power spectra
            P = P.^(1/4); % transform chi-square distributed values to normality (Hawkins and Wixley 1986)
            P = (P-mean(P(:)))./std(P(:)); % normalize to account for different electrode impedances
        end
        ...
        for jj = 1:Ntrl
            ...
            if debug
                ...
                plot(log10(freq),Pwin(:,jj),'k'); hold on;
                plot(log10(freq),cy,'b'); axis tight; axis square;
                set(gca,'Box','off','TickDir','out','FontSize',12,'YLim',[-1.5 1.5]);
                line(get(gca,'XLim'),cbbp*ones(1,2),'Color','r');
                xlabel('log(Hz)'); ylabel('zscore(power)');
                pause; close;
            end
        end
    end
